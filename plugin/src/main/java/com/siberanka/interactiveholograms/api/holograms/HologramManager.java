...
public class HologramManager extends Ticked {
    ...
    public HologramManager(InteractiveHolograms decentHolograms) {
        super(20L);
    ...
    public boolean onClick(final @NonNull Player player, final int entityId, final @NonNull ClickType clickType) {
        ...
        for (Hologram hologram : Hologram.getCachedHolograms()) {
            ...
            double dy = hologram.getLocation().getY() - player.getEyeLocation().getY();
    ...
    /** Side-effect-free ownership check before main-thread action execution. */
    public boolean hasEntity(@NonNull Player player, int entityId) {
        for (Hologram hologram : Hologram.getCachedHolograms()) {
