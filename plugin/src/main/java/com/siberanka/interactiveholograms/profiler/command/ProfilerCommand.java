...
package com.siberanka.interactiveholograms.profiler.command;
...
@CommandInfo(
        usage = "/ih profiler help",
        description = "Commands related to the profiler",
        permissions = {Permissions.COMMAND_PROFILER}
)
public class ProfilerCommand extends DecentCommand {

    public ProfilerCommand(DecentProfiler profiler) {
        super("profiler");

        addSubCommand(new ProfilerHelpCommand(this));
        addSubCommand(new ProfilerStartCommand(profiler));
        addSubCommand(new ProfilerStopCommand(profiler));
        addSubCommand(new ProfilerResetCommand(profiler));
        addSubCommand(new ProfilerStatsCommand(profiler));
    }
