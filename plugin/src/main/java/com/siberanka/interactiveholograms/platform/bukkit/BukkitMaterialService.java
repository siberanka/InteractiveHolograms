...
public class BukkitMaterialService implements PlatformMaterialService {
    ...
    public String toMojangNamespacedKey(String materialName) {
        ...
        // Non-vanilla namespaced IDs are resolved lazily by an installed item
        // provider (currently CraftEngine) when the packet is materialized.
        if (materialName.matches("[a-z0-9_.-]+:[a-z0-9_/.-]+")) {
            return materialName;
        }
    ...
    public boolean isItem(String materialName) {
        if (materialName != null && materialName.matches("[a-z0-9_.-]+:[a-z0-9_/.-]+")
                && !materialName.startsWith(MINECRAFT_NAMESPACE)) {
            return true;
        }
