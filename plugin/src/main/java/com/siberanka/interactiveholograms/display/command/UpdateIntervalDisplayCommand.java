...
@CommandInfo(
        usage = "/ih d update-interval <name> <interval>",
...
class UpdateIntervalDisplayCommand extends DecentCommand {
    ...
    UpdateIntervalDisplayCommand(DisplayService displayService) {
    ...
    public CommandHandler getCommandHandler() {
        return (sender, args) -> {
            ...
            int interval = Validator.getInteger(args[1], 1, 1200, "Interval must be between 1 and 1200.");

            DisplaySettings settings = display.getSettings();
            if (settings.getUpdateInterval() != interval) {
                settings.setUpdateInterval(interval);
                displayService.saveDisplay(display);
            }

            Lang.DISPLAY_UPDATE_INTERVAL_SET.send(sender, display.getName());
            return true;
        };
    }
