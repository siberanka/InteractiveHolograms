...
@CommandInfo(
        usage = "/ih d enable <name>",
        description = "Enable a display if it is disabled.",
        permissions = {Permissions.COMMAND_DISPLAYS_ENABLE},
        minArgs = 1
)
class EnableDisplayCommand extends DecentCommand {

    private final DisplayService displayService;

    EnableDisplayCommand(DisplayService displayService) {
        super("enable");
    ...
    public CommandHandler getCommandHandler() {
        return (sender, args) -> {
            ...
            Lang.DISPLAY_ENABLED.send(sender, display.getName());
            return true;
        };
    }
