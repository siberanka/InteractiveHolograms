...
@CommandInfo(
        usage = "/ih d swap-lines <display> <line1> <line2>",
        description = "Swaps the positions of two lines in a text display.",
        permissions = {Permissions.COMMAND_DISPLAYS_TEXT_SWAP_LINES},
        aliases = {"swaplines"},
        minArgs = 3
)
class TextDisplaySwapLineCommand extends DecentCommand {

    private final DisplayService displayService;
    private final DisplayTabCompleteHelper tabCompleteHelper;

    TextDisplaySwapLineCommand(DisplayService displayService, DisplayTabCompleteHelper tabCompleteHelper) {
        super("swap-lines");
