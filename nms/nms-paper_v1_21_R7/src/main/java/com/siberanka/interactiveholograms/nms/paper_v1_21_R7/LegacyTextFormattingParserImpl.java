...
import com.siberanka.interactiveholograms.nms.api.text.ComponentFormat;
import com.siberanka.interactiveholograms.nms.api.text.LegacyTextFormattingParser;
...
final class LegacyTextFormattingParserImpl extends LegacyTextFormattingParser<MutableComponent, ChatFormatting> {
    ...
    private final Int2ObjectMap<Style> styleCache = new Int2ObjectOpenHashMap<>(1024);
    ...
    protected MutableComponent createFormattedComponent(String text, ComponentFormat format) {
        ...
        Style modifier = getStyle(format);
        component.setStyle(modifier);

        return component;
    }

    private Style getStyle(ComponentFormat format) {
        Style modifier = styleCache.get(format.getKey());
        if (modifier == null) {
            modifier = createStyle(format);
            styleCache.put(format.getKey(), modifier);
        }
        return modifier;
    }
    ...
    private Style createStyle(ComponentFormat format) {
        ...
        Style modifier = Style.EMPTY.applyFormats(formats);
        if (format.getColor() != -1) {
            modifier = modifier.withColor(format.getColor());
        }
        return modifier;
    }
